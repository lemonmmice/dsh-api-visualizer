...
`
    ...
    function buildOpenApi(records) {
      ...
      for (const r of records) {
        ...
        const reqType = String((r.reqHeaders ?? {})['content-type'] ?? 'application/json').split(';')[0]
        ...
        if (op.responses[status] === undefined) {
          const resType = String((r.resHeaders ?? {})['content-type'] ?? 'application/json').split(';')[0]
    ...
    function buildPanel() {
      ...
      wfLegend.appendChild(el('span', 'apv-note', '点击横条打开该请求详情'))
      ...
      const renderWaterfall = (records) => {
        ...
        for (const r of records) {
          ...
          const meta = el('div', 'apv-wf-meta', `${fmtTime(r.ts)} · ${r.method} ${statusBadge(r.status).textContent} · ${fmtDur(r.durationMs)}${who !== '' ? ' · ' + who : ''}`)
      ...
      const renderRules = (data) => {
        ...
        const actionText = (r) => {
          ...
          if (a.type === 'delay') return `延迟 ${a.latencyMs ?? 0}ms`
