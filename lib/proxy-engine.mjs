...
function matchRule(rule, req) {
  ...
  if (m.host && String(m.host).trim() !== '' && String(req.host) !== String(m.host).trim()) return false
...
function tunnelConnect({ upstream, host, port, secure }, cb, timing = null) {
  ...
  const wrapTls = (socket) => {
    ...
    t.once('secureConnect', () => {
      if (timing !== null) timing.tlsMs = Date.now() - timing.t0
      if (done) {
        t.destroy()
        return
      }
...
export class ProxyEngine {
  ...
  loadRules() {
    ...
    if (this.rulesCache.key === key) return this.rulesCache.rules
    let rules = []
    if (key !== 'missing') {
      try {
  ...
  setRules(list) {
    const rules = (Array.isArray(list) ? list : []).map((raw) => ({
      ...
      action: {
        ...
        status: Number.isInteger(raw.action?.status) ? raw.action.status : (raw.action?.type === 'mock' ? 200 : undefined),
  ...
  streamResponse({ record, fwd, res, started, note, rule = null }) {
  ...
  async handlePlain(req, res) {
    ...
    try {
      ...
      await this.streamResponse({ record, fwd, res, started, note: ruleNote ?? '本地代理捕获', rule: rule?.action?.type === 'rewrite-status' ? rule : null })
    } catch (error) {
  ...
  async handleTunneledRequest(host, port, req, res) {
    ...
    try {
      ...
      await this.streamResponse({ record, fwd, res, started, note: ruleNote ?? '本地代理捕获(HTTPS 解密)', rule: rule?.action?.type === 'rewrite-status' ? rule : null })
    } catch (error) {
      this.counters.errors++
  ...
  handleWsUpgrade(req, socket, head, ctx = null) {
    ...
    const headLines = [`${req.method} ${ctx !== null && ctx.tunnel ? u.pathname + u.search : req.url} HTTP/1.1`]
    ...
    tunnelConnect({ upstream: this.upstream, host, port, secure }, (error, up) => {
      ...
      const armIdle = () => {
        if (idleTimer !== null) clearTimeout(idleTimer)
        idleTimer = setTimeout(() => finish(false), 120000)
      }
      armIdle()
  ...
  start({ port = this.port, upstream } = {}) {
    ...
    this.server.on('upgrade', (req, socket, head) => this.handleWsUpgrade(req, socket, head))
